...
abi_variables_output_random_sample = ['CMIP_min', 'CMIP_05', 'ACHA_max', 'ACHA_95', 'abi_lat', 'abi_lon', 'fistart_flid'] 
...
def abi_driver(abi, ff, row):
    ...
    abi_lats, abi_lons, acha_vals, cmip_vals = mamf.abi_file_loader(acha_file,cmip_file)
    ...
    if row['ma_category'] != 'Miss':
        ...
        if random_df.shape[0]>0:
            ...
            fi_fl = [row['fistart_flid'] for i in range(abi_random_df.shape[0])]
...
def mrms_driver(mrms, ff, row, random_df):
    ...
    for var in mrms_variables:
        ...
        if mrms_lats[0]==-999:
            ...
            mrms.loc[row['fistart_flid'],var+'_max'] = var_max
            mrms.loc[row['fistart_flid'],var+'_95'] = var_95
    
    #If the ma_category is a hit/late, then use it to randomly sample the ABI scene
    if (row['ma_category'] != 'Miss')and(random_df.shape[0]>0):
