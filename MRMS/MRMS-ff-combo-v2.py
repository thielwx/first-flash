...
# This was based on MRMS-ff-combo-v1.py but added in sampling 10 minutes BEFORE the first flash
#
# Created: September 2025
# Author: Kevin Thiel (kevin.thiel@ou.edu)
#
# Run like this: python MRMS-ff-combo-v2.py /localdata/first-flash/data/GLM16-ff-processed/*FILENAMEHERE*.nc
#===================================================
...
mrms_variables_output = ['MergedReflectivityQCComposite_max','MergedReflectivityQCComposite_95','Reflectivity_-10C_max','Reflectivity_-10C_95','ReflectivityAtLowestAltitude_max','ReflectivityAtLowestAltitude_95','MergedReflectivityQCComposite_max_pre10','MergedReflectivityQCComposite_95_pre10','Reflectivity_-10C_max_pre10','Reflectivity_-10C_95_pre10','ReflectivityAtLowestAltitude_max_pre10','ReflectivityAtLowestAltitude_95_pre10']
version = 2
...
def mrms_driver(t_start, t_end):
    ...
    #A 2-minute time list to go through each mrms file, plus the one 10 minutes before
    tlist_2min = pd.date_range(start=t_start, end=t_end, freq='120s')
    tlist_2min_pre10 = tlist_2min - timedelta(minutes=10)    
    ...
    for i in range(len(tlist_2min)-1):
        ...
        if len(ff_locs)>0: #If there's first flashes in the 2 minute span, we'll get the MRMS values
            ...
            for var in mrms_variables:
                mrms_lats, mrms_lons, mrms_data = MRMS_data_loader(tlist_2min[i], fstring_start, var, False)
                mrms_lats_pre10, mrms_lons_pre10, mrms_data_pre10 = MRMS_data_loader(tlist_2min_pre10[i], fstring_start, var, True)
                ...
                for ff in ff_locs:
                    ...
                    var_max_pre10, var_95_pre10 = mrms_max_finder(cur_fl_lat, cur_fl_lon, mrms_lats_pre10, mrms_lons_pre10, mrms_data_pre10)

                    df.loc[cur_fi_fl,var+'_max'] = var_max
                    df.loc[cur_fi_fl,var+'_95'] = var_95
                    df.loc[cur_fi_fl,var+'_max_pre10'] = var_max_pre10
                    df.loc[cur_fi_fl,var+'_95_pre10'] = var_95_pre10
...
def MRMS_data_loader(time, fstring_start ,var, pre_truth):
    ...
    #Creating mechanism to check if we're in the first ten minutes of the day and need to look at yesterday's MRMS for the pre-10 minute data instead...
    if pre_truth == True:
        if int(hr)==23 and int(mi)>=50:
            fstring_start = '/raid/swat_archive/vmrms/CONUS/'+y+m+d+'/multi/'

    #Completing the entire time string
